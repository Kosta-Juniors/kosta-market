...
public interface ProductMapper {
    ...
    // 상품 리스트 불러오기
    @Select("SELECT A.product_id, A.product_name, A.product_price, " +
    ...
    int insertComment(@Param("orderId") int orderId, @Param("productId") int productId, @Param("score") int score,
    ...
    int deleteComment(@Param("userId") int userId, @Param("commentId") Integer commentId);

    //** 수정할 댓글 관련 정보 가져오기
    @Select("SELECT * FROM TBL_COMMENT " +
            "WHERE deleted LIKE 'N' " +
            "AND comment_id=#{commentId}")
    CommentDto selectComment(@Param("commentId") int commentId);

    // commentId가 유효한지 검사
    @Select("SELECT COUNT(*) FROM TBL_COMMENT WHERE comment_Id=#{commentId}")
    int selectCountById(@Param("commentId")int commentId);
    ...
    @Update("UPDATE TBL_COMMENT AS A " +
            ...
            "AND C.user_id=#{userId}")
    int updateComment(@Param("userId") int userId, @Param("orderId") int orderId, @Param("subQuery") String subQuery);
    ...
    @Select("SELECT A.comment_id, A.order_id, A.product_id, D.name,A.score,A.content,A.created_at " +
            ...
            "LIMIT #{offset},#{size}")
    List<CommentListDto> selectCommentList(@Param("productId") int productId, @Param("offset") int offset,
                                           @Param("size") int size, @Param("subQuery") String subQuery);

    //** 하나의 주문에 하나의 상품만 담길 수 있도록 확인하는 것
    @Select("SELECT COUNT(*) FROM TBL_COMMENT " +
            "WHERE order_id=#{orderId}")
    int selectDuplicateComment(@Param("orderId") int orderId);


    //** 상품 댓글 관련 개수
    @Select("SELECT COUNT(*) FROM TBL_COMMENT WHERE deleted LIKE 'N' AND product_id=#{productId}")
    Integer selectCommentCountByproductId(@Param("productId") int productId);
    ...
    @Select("SELECT A.product_id, A.product_name, A.product_price, " +
            ...
            "ORDER BY rand() " +
            "LIMIT 5")
    List<ProductDto> selectProductByCategoryId(@Param("categoryId") int categoryId);
