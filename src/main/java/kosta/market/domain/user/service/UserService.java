...
public interface UserService {
    ...
    /**
     * 기능 : 로그인 처리 아이디, 패스워드가 일치하면 로그인
     *
     * @param userCheckDto 로그인 시 DB와 비교할 아이디, 패스워드가 담긴 Dto
     * @param session      일반 유저라면 key = "user_id", value = user_id 세션 생성 판매자라면 key = "seller_id",
     *                     value = seller_id 추가적으로 세션 생성
     * @return 로그인이 성공하면 true, 실패했다면 false
     */
    boolean loginUser(UserCheckDto userCheckDto, HttpSession session);

    /**
     * 기능 : 회원 가입 처리 등록할 내용 : 아이디(username), 패스워드(password), 이름(name), 연락처(contact) user_id는 자동으로
     * 등록
     *
    ...
    /**
     * 기능 : 회원정보 불러오기 확인 가능한 정보 : user_id, 아이디(username), 패스워드(password), 이름(name), 연락처(contact)
     *
    ...
    /**
     * 기능 : 배송지 등록 is_default_address '1' -> 현재 배송지 배송지가 추가되면 update를 통해 is_default_address 값이 '0'
     * 으로 설정하고, insert를 통해 is_default_address 값을 '1'로 설정함 새로운 배송지가 추가되면 기존의 is_default_address의 값은 0
     * 으로 변하게 됨
     *
     * @param addressDto 배송지와 관련된 user_id, address_id, delively_place, is_default_address를 담고 있는
     *                   Dto
    ...
    /**
     * 기능 : 배송지 불러오기 확인 가능한 정보 : user_id, address_id, delively_place, is_default_address
     *
    ...
    /**
     * 기능 : 판매자 등록 판매자 등록하면서 key = "seller_id", value = seller_id 생성
     *
